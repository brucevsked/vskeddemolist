...
public class WeChatUtils {
     ...
     * @return
     */
    public static String unifiedOrder1(String openid, String orderSn, Integer price, String notifyUrl, Integer ktvId,
			Integer roomId, Long ktvScreenplayId, Integer userPayPricel) {
        try {
            Kv amount = Kv.by("total", price).set("currency", "CNY");
            Kv payer = Kv.by("openid", openid);
            ...
            params.put("create_time", new Date());//创建时间
            params.put("out_trade_no", orderSn);// 订单编号
            params.put("ktv_id", ktvId);// ktv的id
            params.put("room_id", roomId);// ktv的id
            params.put("ktv_screenplay_id", ktvScreenplayId);// ktv的id
            params.put("status", "10");// 订单状态
            params.put("order_price", userPayPricel);// 订单状态
            params.put("wx_transaction_id", "微信订单号");// 微信订单号
            params.put("wx_outTrade_no", "微信交易流水号");// 微信交易流水号
            params.put("flow_id", "流程的ID");// 流程的ID
            params.put("play_status", "游玩状态");// 游玩状态
            params.put("flow_all", "已经历的流程");// 已经历的流程
            params.put("is_refund", "是否已退款");// 是否已退款 0 否 1 是
            params.put("flow_update_time", "流程更新时间");// 流程更新时间
