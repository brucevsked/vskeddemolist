...
public class WxTool {
    ...
    public static String getAccessToken(){
        ...
        try{
            redisAccessToken=Redis.use("ktvdata").get("ktv1:wx:accesstoken");
            log.info("|{}|",redisAccessToken);
            //先看缓存有没有
            if(redisAccessToken!=null){
                return redisAccessToken;
            }
        }catch (Exception e){
           log.error("get redis access token error:",e);
        }

        redisAccessToken=getAccessTokenNative();
        log.info("|{}|",redisAccessToken);
        Redis.use("ktvdata").setex("ktv1:wx:accesstoken",7000,redisAccessToken);
