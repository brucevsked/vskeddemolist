...
<body>
 ...
 <a href="javascript:void(0);" onclick="dosubmit('fm',${p.currentPage-1})" >上一页</a></c:if>
    第<input type="text" id="currentPage" name="currentPage" value="${p.currentPage }" size="1">页
 <c:if test="${p.currentPage<p.numCount}">
 <a href="javascript:void(0);" onclick="dosubmit('fm',${p.currentPage+1})">下一页</a>	
  <a href="javascript:void(0);" onclick="dosubmit('fm',${p.numCount})" >尾页</a>
 </c:if>
    共${p.numCount }页&nbsp; 
 <select id="pageSize" name="pageSize" onchange="dosubmit('fm',${p.currentPage })">
