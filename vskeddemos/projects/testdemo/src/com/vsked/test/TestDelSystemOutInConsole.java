...
/**
 * use delete the System.out.print in project
 * @author vsked 201302180856
 *
 */
public class TestDelSystemOutInConsole {
	...
	public static String readFileByLine( File inF) throws Exception{
		...
		{
		    tmp+=data.replace(getSystemOut(data,false), "")+"\n";
		}
		tmp=tmp.replace(getSystemOut(tmp,true), "");
		fis.close();
		return tmp;
	}
	...
	public static String getSystemOut(String s,boolean isEnd){
		String exp="System.out(.)*\\;";
		if(isEnd)
			exp="/\\*\\s(.)*\\s*(.)*\\s*(.)*\\s*(.)*";
