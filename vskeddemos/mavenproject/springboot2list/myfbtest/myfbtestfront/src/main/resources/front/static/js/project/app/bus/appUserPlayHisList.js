...
$(function(){
	$('#queryBt').on('click',appUserPlayHisList.query);
	appUserPlayHisList.list();
});

var appUserPlayHisList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appuserplayhislist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.phone=$('#phone').val();
