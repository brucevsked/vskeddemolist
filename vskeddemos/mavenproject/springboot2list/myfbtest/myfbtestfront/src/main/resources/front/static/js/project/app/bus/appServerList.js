...
$(function(){
	$('#queryBt').on('click',appServerList.query);
	appServerList.list();
});

var appServerList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appserverlist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.group=$('#group').val();
                	d.name=$('#name').val();
		            ...
		            text:'修改',
		  	        action:function(e, dt, node, config){
		  	        	...
		  	        	if(selectData.length>0){
		  	        		appServerList.toUpdate(selectData[0].id);
		  	        	}else{
		  	        		toastr.info('请选择数据');
		  	        	}
		  	        }
    ...
    toAdd:function(){
		webCommon.loadPage('/webapi/v1/toappserveradd',{token: $('#servertoken').val() });
	},
	toUpdate:function(fid){
		webCommon.loadPage('/webapi/v1/toappserverupdate',{token: $('#servertoken').val(),fid:fid });
	}
});