...
$(function(){
	$('#queryBt').on('click',appUserChatHisList.query);
	appUserChatHisList.list();
});

var appUserChatHisList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appuserchathislist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.phone=$('#phone').val();
		  	...
		  	columns:[
		  	    ...
		  	    {data:'msgcontent',
			  	 render:function(data, type, full, meta){
			  		 var myDataMap=$.parseJSON(data);
			  		 var rs=myDataMap.content;
			  	   	 return rs;
			  	}},
		  	    {data:'addtime',
		  	   	 render:function(data, type, full, meta){
