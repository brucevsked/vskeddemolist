...
$(function(){
	$('#queryBt').on('click',appHumanList.query);
	appHumanList.list();
});

var appHumanList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/apphumanlist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.hname=$('#hname').val();
                	d.hdescription=$('#hdescription').val();
                },
                dataType: 'json'
			},
		    ...
		    buttons:[
		            ...
		            text:'修改',
		  	        action:function(e, dt, node, config){
		  	        	...
		  	        	if(selectData.length>0){
		  	        		appHumanList.toUpdate(selectData[0].id);
		  	        	}else{
		  	        		toastr.info('请选择数据');
		  	        	}
		  	        }
		  	...
		  	columns:[
		  	    ...
		  	    {data:'type1name'},
		  	    {data:'description'},
		  	  	{data:'pic', render:function(data, type, full, meta){
    ...
    toAdd:function(){
		webCommon.loadPage('/webapi/v1/toapphumanadd',{token: $('#servertoken').val() });
	},
	toUpdate:function(fid){
		webCommon.loadPage('/webapi/v1/toapphumanupdate',{token: $('#servertoken').val(),fid:fid });
	}
});