...
$(function(){
	$('#queryBt').on('click',appModuleList.query);
	appModuleList.initDate();
	appModuleList.initUI();
	appModuleList.list();
});

var appModuleList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appmodulelist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.mname=$('#mname').val();
                	d.type=$('#type').val();
		            ...
		            text:'修改',
		  	        action:function(e, dt, node, config){
		  	        	...
		  	        	if(selectData.length>0){
		  	        		appModuleList.toUpdate(selectData[0].id);
		  	        	}else{
		  	        		toastr.info('请选择数据');
		  	        	}
		  	        }
		  	        		...
		  	        		swal({
		                        ...
		                        closeOnCancel: true 
		                        	...
		                        	if (isConfirm) {
		    		  	        		...
		    		  	        		var myBasePath='/webapi/v1/appmoduledel';
    ...
    toAdd:function(){
		webCommon.loadPage('/webapi/v1/toappmoduleadd',{token: $('#servertoken').val() });
	},
	toUpdate:function(fid){
		webCommon.loadPage('/webapi/v1/toappmoduleupdate',{token: $('#servertoken').val(),fid:fid });
	},
	initUI:function(){
		$.post('/webapi/v1/appmoduletypelistdata',{token:$('#servertoken').val(),level:1},function (dt){
			var tmpArray=new Array();
			$.each(dt,function(index,el){
			  tmpArray.push({id:el.id,text:el.name});
			});
			$('#type').select2({width: '100%',data: tmpArray});
			$('#type').append('<option value="" selected>请选择模块类型</option>');
			$('#type').trigger('change');
		},'json');
