...
$(function(){
	$('#queryBt').on('click',appUserCollectList.query);
	appUserCollectList.list();
});

var appUserCollectList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appusercollectlist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.phone=$('#phone').val();
