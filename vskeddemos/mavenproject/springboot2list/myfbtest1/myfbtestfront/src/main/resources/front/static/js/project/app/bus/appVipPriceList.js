...
$(function(){
	$('#queryBt').on('click',appVipPriceList.query);
	appVipPriceList.list();
});

var appVipPriceList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appvippricelist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.vpname=$('#vpname').val();
                	d.vprice=$('#vprice').val();
		            ...
		            text:'修改',
		  	        action:function(e, dt, node, config){
		  	        	...
		  	        	if(selectData.length>0){
		  	        		appVipPriceList.toUpdate(selectData[0].id);
		  	        	}else{
		  	        		toastr.info('请选择数据');
		  	        	}
		  	        }
    ...
    toAdd:function(){
		webCommon.loadPage('/webapi/v1/toappvippriceadd',{token: $('#servertoken').val() });
	},
	toUpdate:function(fid){
		webCommon.loadPage('/webapi/v1/toappvippriceupdate',{token: $('#servertoken').val(),fid:fid });
	}
});