...
$(function(){
	$('#queryBt').on('click',appUserBuyList.query);
	appUserBuyList.list();
});

var appUserBuyList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appuserbuylist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.phone=$('#phone').val();
