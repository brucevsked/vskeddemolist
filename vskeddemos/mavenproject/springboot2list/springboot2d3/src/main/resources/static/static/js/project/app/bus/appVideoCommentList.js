...
$(function(){
	$('#queryBt').on('click',appVideoCommentList.query);
	appVideoCommentList.list();
});

var appVideoCommentList=new Object({
	list: function(){
		var myTable=$('#dt0').DataTable({
			ajax:{
				url:'/webapi/v1/appvideocommentlist?token='+$('#servertoken').val(),
                type:'post',
                data:function(d){
                	d.vmcontent=$('#vmcontent').val();
                	d.vmphone=$('#vmphone').val();
                },
                dataType: 'json'
			},
		  	        		...
		  	        		swal({
		                        ...
		                        closeOnCancel: true 
		                        	...
		                        	if (isConfirm) {
		    		  	        		...
		    		  	        		var myBasePath='/webapi/v1/appvideocommentdel';
		    		  	        		if(myIdsArray.length>0){
		    		  	        			$.post(myBasePath,{token:$('#servertoken').val(),ids:myIdsArray.join(',')},function (dt){
		    		  	        				if('000000'==dt.code){
		    		  	        					swal("删除", "选中影片评论已被删除.", "success");
		    		  	        					appVideoCommentList.query();
				    		  	  			    }else{
				    		  	  				    swal("取消", "选中影片评论删除失败:"+dt.msg, "error");
				    		  	  			    }
