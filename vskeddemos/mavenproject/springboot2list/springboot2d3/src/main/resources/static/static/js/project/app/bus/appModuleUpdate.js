...
$(function(){
	$('#updateBt').on('click',appModuleUpdate.update);
    webCommon.initICheck();
    appModuleUpdate.initUI();
    $('#moduleName').focus();
});

var appModuleUpdate=new Object({
	updateIsClick:true,
	oldSelectedOptiont1:null,
	update:function(){
		if(appModuleUpdate.updateIsClick==false) return false;
		var fid=$('#fid').val();
		...
		var myBasePath='/webapi/v1/appmoduleupdate';
		...
		appModuleUpdate.updateIsClick=false;
		setTimeout(function() {	appModuleUpdate.updateIsClick = true;	}, 3000);//3秒内不可以重复点击
	...
	initUI:function(){
		$.post('/webapi/v1/appmoduletypelistdata',{	token:$('#servertoken').val()},function (dt){
			...
			$.each(dt,function (myIndex,myEl){
				var tmpDt={id:myEl.id,text:myEl.name};
			    tmpArray.push(tmpDt);
			    if(tmpDt.id==$('#oldModuleType').val()){
			    	appModuleUpdate.oldSelectedOptiont1=tmpDt;
			    }
			});
			$('#appModuleTypeId').select2({width: '60%',data: tmpArray});
			if(appModuleUpdate.oldSelectedOptiont1!=null){
			 $('#appModuleTypeId').val(appModuleUpdate.oldSelectedOptiont1.id,appModuleUpdate.oldSelectedOptiont1.text).trigger("change"); 
			}	
	    },'json');
